...
FLOW: tuple[FlowStep, ...] = (
    FlowStep(
        ...
        apis=("oauth.authorize", "taobao.top.auth.token.create"),
    ...
    FlowStep(
        ...
        title="投料",
        actor="user",
        user_input="拖入 1～6 张图；建议再填货号、FOB/售价、MOQ。规格书可选",
        apis=(),
        notes="这是用户每条商品真正要做的事。没有图就无法识别，也发不了品。",
    ),
    FlowStep(
        id="understand",
        title="AI 看图理解",
    ...
    FlowStep(
        ...
        apis=("alibaba.icbu.category.get.new",),
    ...
    FlowStep(
        ...
        title="生成英文标题 / 关键词 / 详情",
        actor="ai",
        user_input="无，高置信度直接用",
        apis=(),
        schema_fields=("productTitle", "productKeywords", "superText"),
        notes="标题 ≤128 字节、关键词 1～3 个、禁中文、禁邮箱、禁 HTML。先质检再给用户看。",
    ),
    FlowStep(
        id="upload_photos",
        title="图片进图片银行",
    ...
    FlowStep(
        ...
        title="审稿（只看红黄项）",
        actor="user",
        user_input="确认低置信度类目/属性；核对价格和 MOQ",
        apis=(),
        schema_fields=USER_FIELDS,
        notes="绿项折叠。前 20 条建议看一眼类目，之后同类目可记住不再问。",
    ),
    FlowStep(
        id="publish",
        title="排队发布",
        ...
        apis=("alibaba.icbu.product.schema.add",),
